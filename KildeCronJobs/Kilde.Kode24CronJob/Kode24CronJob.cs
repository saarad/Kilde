...
{
    public class Kode24CronJob : IHostedService
    {
        private readonly IServiceProvider _serviceProvider;

        public Kode24CronJob(IServiceProvider serviceProvider)
        {
            ...
            while (true)
            {
                while (true)
                {
                    ...
                    using (var scope = _serviceProvider.CreateScope()) //keep unsmiplified to avoid dev errors
                    {
                        ...
                        logger.LogInformation("Starting work on Kode24");

                        var articleService = _serviceProvider.GetRequiredService<IArticleService>();
                        var kode24Service = new Kode24Service(logger, articleService);
